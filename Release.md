...
- **MarkerTracking3D Rewrite (2D + Triangulation + RANSAC)**: Replaced the brute-force 3D grid search with a hybrid 2D-per-camera + RANSAC triangulation approach. Per camera, the previous frame's 3D position is projected to define a search window, NCC template matching extracts up to 3 peak candidates, and a RANSAC-like exhaustive search over camera-pair × candidate combinations triangulates then scores by reprojected NCC consistency + distance-from-prediction regularization. The best 3D point is refined via local hill-climbing in 3D, then `MarkerDetection::detectionPoint` snaps each 2D projection to sub-pixel precision. This runs in a background thread and handles marker reversals naturally (0th-order prediction) without the exponential memory cost of a 3D grid.
